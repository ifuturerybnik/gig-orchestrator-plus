// OAuth 2.0 callback dla Spotify.
// Stały URL: ${origin}/api/public/social/spotify-callback
import { createFileRoute } from "@tanstack/react-router";
import { handleSpotifyOAuthCallback } from "@/lib/social-oauth.server";
...
export const Route = createFileRoute("/api/public/social/spotify-callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (errorParam) {
          return html(
            ...
            `<p>Spotify zwrócił błąd: <b>${errorParam}</b></p>`,
            false,
          );
        }
        if (!code || !state) {
        ...
        const callbackUrl = `${url.origin}/api/public/social/spotify-callback`;
        try {
          const res = await handleSpotifyOAuthCallback({ code, state, callbackUrl });
          ...
          return html("Błąd podczas łączenia ze Spotify", `<p>${msg}</p>`, false);
        }
      },
    },
  },
