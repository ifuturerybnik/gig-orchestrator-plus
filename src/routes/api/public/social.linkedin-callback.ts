// OAuth 2.0 callback dla LinkedIn.
// Stały URL: ${origin}/api/public/social/linkedin-callback
import { createFileRoute } from "@tanstack/react-router";
import { handleLinkedInOAuthCallback } from "@/lib/social-oauth.server";
...
export const Route = createFileRoute("/api/public/social/linkedin-callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (errorParam) {
          return html(
            ...
            `<p>LinkedIn zwrócił błąd: <b>${errorParam}</b></p><p>${errorDesc ?? ""}</p>`,
            false,
          );
        }
        if (!code || !state) {
        ...
        const callbackUrl = `${url.origin}/api/public/social/linkedin-callback`;
        try {
          const res = await handleLinkedInOAuthCallback({ code, state, callbackUrl });
          ...
          return html("Błąd podczas łączenia z LinkedIn", `<p>${msg}</p>`, false);
        }
      },
    },
  },
