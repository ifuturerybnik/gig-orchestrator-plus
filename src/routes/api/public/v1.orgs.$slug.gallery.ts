...
export const Route = createFileRoute("/api/public/v1/orgs/$slug/gallery")({
  server: {
    handlers: {
      ...
      GET: async ({ request, params }) => {
        ...
        const { data, error } = await supabaseAdmin
          ...
          .select(
            "id, slug, title_i18n, description_i18n, cover_image_url, event_id, published_at",
          )
        ...
        const albums = (data ?? []).map((r) => ({
          ...
          description: localize(r.description_i18n as Record<string, string>, lang),
          cover_image_url: r.cover_image_url,
          event_id: r.event_id,
          published_at: r.published_at,
        }));
        return jsonResponse({ lang, count: albums.length, albums }, cors);
      },
    },
  },
});
