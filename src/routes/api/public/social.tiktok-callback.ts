// OAuth 2.0 callback dla TikTok.
// Stały URL: ${origin}/api/public/social/tiktok-callback
import { createFileRoute } from "@tanstack/react-router";
import { handleTikTokOAuthCallback } from "@/lib/social-oauth.server";
...
export const Route = createFileRoute("/api/public/social/tiktok-callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (errorParam) {
          return html(
            ...
            `<p>TikTok zwrócił błąd: <b>${errorParam}</b></p><p>${errorDesc ?? ""}</p>`,
            false,
          );
        }
        if (!code || !state) {
        ...
        const callbackUrl = `${url.origin}/api/public/social/tiktok-callback`;
        try {
          const res = await handleTikTokOAuthCallback({ code, state, callbackUrl });
          ...
          return html("Błąd podczas łączenia z TikTok", `<p>${msg}</p>`, false);
        }
      },
    },
  },
