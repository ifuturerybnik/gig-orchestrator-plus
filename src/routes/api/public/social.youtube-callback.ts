// OAuth 2.0 callback dla YouTube (Google).
// Stały URL: ${origin}/api/public/social/youtube-callback
import { createFileRoute } from "@tanstack/react-router";
import { handleYouTubeOAuthCallback } from "@/lib/social-oauth.server";
      ...
      GET: async ({ request }) => {
        ...
        if (errorParam) {
          return html(
            ...
            `<p>Google zwrócił błąd: <b>${errorParam}</b></p>`,
            false,
          );
        }
        if (!code || !state) {
        ...
        try {
          ...
          return html(
            "Połączono z YouTube",
            `<p>Kanał <b>${res.accountName}</b> został powiązany z Twoją organizacją.</p><p>Za chwilę wrócisz do aplikacji…</p>`,
            true,
            back,
          );
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Nieznany błąd";
          console.error("[youtube-callback]", msg);
          return html("Błąd podczas łączenia z YouTube", `<p>${msg}</p>`, false);
        }
      },
    },
  },
