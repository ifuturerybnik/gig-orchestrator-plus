-- ============================================================================
-- 0024_performances_data_api_diagnostic.sql
-- Naprawa + diagnostyka dla błędu Data API:
--   Could not find the table 'public.performances' in the schema cache
--
-- WAŻNE: uruchom ten plik w SQL Editor dokładnie w tym projekcie Supabase,
-- do którego wskazuje aplikacja:
--   https://rpnucwqjtnxfflwqpcgg.supabase.co
-- ============================================================================
...
-- Oficjalny workaround Supabase dla uporczywie nieodświeżonego cache PostgREST.
select pg_notification_queue_usage() as pgrst_notification_queue_usage;

-- Wynik kontrolny: po uruchomieniu powinny być dwa wiersze z exists_in_db = true.
select
  current_database() as database_name,
  current_schema() as current_schema,
  table_name,
  to_regclass('public.' || table_name) is not null as exists_in_db,
  ...
  has_table_privilege('service_role', 'public.' || table_name, 'select') as service_role_can_select
from (values ('performances'), ('performance_assignments')) as t(table_name);