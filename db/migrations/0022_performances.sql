-- ============================================================================
-- 0022_performances.sql — Moduł "Występy" organizacji
-- ============================================================================
-- Tabela występów (koncerty/eventy) + przypisania kontakt/kontrahent.
-- ============================================================================
...
comment on table public.performances is
  'Występy / koncerty organizacji. Walidacja warunkowa (status, widoczność) w warstwie aplikacji.';

-- Przypisania (kontakt LUB kontrahent)
...
-- assignments — RLS przez relację do performances
  ...
  using (
    exists (
      ...
      where p.id = performance_id
        and (
          public.is_member_of(auth.uid(), p.organization_id)
          or public.is_admin(auth.uid())
        )
    )
  );
